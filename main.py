...
async def panel(event):
    if event.chat_id != ADMIN_ID:
        return
    await event.reply("🎛 **GoodShort Bot Control Panel**", buttons=get_panel_buttons())
...
async def panel_callback(event):
    ...
    try:
        if data == b"start_auto":
            ...
            await event.edit("🎛 **GoodShort Bot Control Panel**", buttons=get_panel_buttons())
...
async def process_drama_full(book_id, chat_id, status_msg=None):
    """Refactored logic for GoodShort API."""
    data_full = await get_drama_detail(book_id)
    if not data_full:
    ...
    try:
        if status_msg: await status_msg.edit(f"🎬 Processing **{title}** (GoodShort)...")
...
async def auto_mode_loop():
    ...
    while True:
        ...
        try:
            ...
            logger.info(f"🔍 [GoodShort] Scanning for new dramas...")
            ...
            for drama in all_new:
                ...
                try:
                    await client.send_message(ADMIN_ID, f"🆕 **Auto-System GShort**\n🎬 `{title}`\n🆔 `{bid}`\n⏳ Memproses...")
                except: pass
...
if __name__ == '__main__':
    logger.info("GoodShort Bot Active.")
